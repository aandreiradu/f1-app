		...
		const getScheduleDetailsByID = async () => {
			...
			try {
				const storedCircuitData =
					circuitScheduleStore?.find((cc) => cc?.Circuit?.circuitId === circuitId) || null;
				...
				if (storedCircuitData) {
					...
					sendRequest(
						...
						(responseData) => {
							console.log(`responseData for circuit ${circuitId}`, responseData);
							if (isMounted) {
				...
				console.log(`error on schedule details for circuit ${circuitId}`, error);
								...
								circuitSchedule?.map((item, index) => {
									...
									return (
										<RaceHub
											...
											className="defaultTransition defaultTransition-M1"
										>
											<RaceHeader>
												<RaceImage src={circuitContent?.imgSrc} alt={circuitId} />
												<RaceContent>
													<h1>{item?.Circuit?.Location?.country}</h1>
													<RaceYearWrapper>
														<RaceYearImage
															src={
																scheduleCircuits.find((img) => img?.circuitId === '2022')?.imgSrc
															}
															alt={'year'}
														/>
													</RaceYearWrapper>
											...
											<section>
												...
												<RaceScheduleListing>
													...
													<CircuitScheduleItem
														...
														monthName={monthNames[new Date(item?.Qualifying?.date).getMonth()]}
														eventTime={dateUtils.UTCtoRO(
													...
													<CircuitScheduleItem
														title="Practice 3"
														dayNo={new Date(item?.ThirdPractice?.date).getDate()}
														monthName={monthNames[new Date(item?.ThirdPractice?.date).getMonth()]}
														eventTime={
															dateUtils.UTCtoRO(
													...
													<CircuitScheduleItem
														title="Practice 2"
														dayNo={new Date(item?.SecondPractice?.date).getDate()}
														monthName={monthNames[new Date(item?.SecondPractice?.date).getMonth()]}
														eventTime={
															dateUtils.UTCtoRO(
													...
													<CircuitScheduleItem
														title="Practice 1"
														dayNo={new Date(item?.FirstPractice?.date).getDate()}
														monthName={monthNames[new Date(item?.FirstPractice?.date).getMonth()]}
														eventTime={
															dateUtils.UTCtoRO(
