...
const ScheduleItem = (props) => {
	...
	let raceNotDisputed = (
		<Link
			to={`/schedule/${round}/${circuitId}`}
			className={`${classes['image-wrapper']} ${classes['notDisputed']}`}
		>
			<img
			...
			<>
				<Position1>
					<ImageWrapper>
						<img
							src={
								driversScheduleImages.find((item) =>
									item.driverId
										.toUpperCase()
										.includes(thisRoundResults ? thisRoundResults?.podiums[0] : '')
								)?.imgSrc
							}
							alt="position 1"
						/>
					...
					<DriverInfoWrapper>
						<DriverInfo>{thisRoundResults ? thisRoundResults.p1 : ''}</DriverInfo>
					</DriverInfoWrapper>
				</Position1>
				<Position2>
					<ImageWrapper>
						<img
							src={
								driversScheduleImages.find((item) =>
									item.driverId
										.toUpperCase()
										.includes(thisRoundResults ? thisRoundResults?.podiums[1] : '')
								)?.imgSrc
							}
							alt="position 2"
						/>
					...
					<DriverInfoWrapper>
						<DriverInfo>{thisRoundResults ? thisRoundResults.p2 : ''}</DriverInfo>
					</DriverInfoWrapper>
				</Position2>
				<Position3>
					<ImageWrapper>
						<img
							src={
								driversScheduleImages.find((item) =>
									item.driverId
										.toUpperCase()
										.includes(thisRoundResults ? thisRoundResults?.podiums[2] : '')
								)?.imgSrc
							}
							alt="position 3"
						/>
					...
					<DriverInfoWrapper>
						<DriverInfo>{thisRoundResults ? thisRoundResults.p3 : ''}</DriverInfo>
					</DriverInfoWrapper>
				</Position3>
			</>
		);
	...
	return (
		<CardSchedule>
			...
			<EventDetails>
				...
				<EventResult>{content}</EventResult>
			</EventDetails>
		</CardSchedule>
	);
};
