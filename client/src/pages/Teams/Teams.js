...
const Teams = () => {
      ...
      try {
        sendRequestDrivers(
          ...
          (driversResponse) => {
            ...
            const driversStandings = driversResponse?.MRData?.StandingsTable?.StandingsLists[0]?.DriverStandings;
        ...
        dispatch(fetchTeamsFailure(error?.message || 'Something went wrong. Please try again later!'));
      }
    }
      ...
      try {
        sendRequestDrivers(
          ...
          (constructorResponse) => {
            ...
            const teamsSorted = (constructorResponse?.MRData?.StandingsTable?.StandingsLists[0]?.ConstructorStandings)?.sort((a,b) => b?.points - a?.points);
  ...
  return (
    <div className={`${classes.wrapper} defaultTransition`}>
      ...
      {
        (errorConstructor || errorDrivers) && showModal && <ErrorModal title='Ooops!' message = {errorConstructor?.message || errorDrivers?.message || 'Something went wrong. Please try again later!'} onConfirm={confirmErrorModal} />
      }
      {
      (isLoadingConstructors || isLoadingDrivers) ? <Loader /> : 
        (
